					...
					enum adsp_reserve_mem_id_t id)
{
	...
	if (id >= ADSP_NUMS_MEM_ID) {
		pr_err("%s no reserve memory for %d\n", __func__, id);
		return NULL;
	}
	if (!va_start) {
		pr_err("%s va_start is NULL\n", __func__);
		return NULL;
	}
...
void adsp_set_emimpu_shared_region(void)
{
	...
	if (ret < 0)
		pr_err("%s fail to init emimpu region\n", __func__);
	...
	if (ret < 0)
		pr_err("%s fail to set emimpu protection\n", __func__);
...
void adsp_init_reserve_memory(void)
{
	...
	if (!mem->phys_addr || !mem->size) {
		pr_err("%s() reserve memory illegal addr:%llx, size:%zx\n",
